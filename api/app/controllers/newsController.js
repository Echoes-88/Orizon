...
const handleQuizController = {
	...
	getAllNews: async(req, res) => {
		...
		for(const result of slicedLemonde) {
			let formatedResultTemp = [];

			formatedResultTemp.push(renameKey(result, "media:content", "media"));


			formatedResultTemp.map(({ title, description, link, media, pubDate }) => {
				let img = media.$.url;
				let source = "Le Monde";
				formatedResult.push({title, description, link, img, pubDate, source});
			});
		...
		// slicedLemonde.map(({ title, description, link, enclosure, pubDate, item }) => {
